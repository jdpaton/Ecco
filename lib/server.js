 ...
 */
var EccoServer = function(opts) {

    if(!(this instanceof EccoServer)) {
        return new EccoServer(opts);
    }
    ...
    this.server = net.createServer(function(c) {
        ...
        if (opts['out-file']) {
            _out_file = fs.createWriteStream(opts['out-file'], {
                flags: 'a'
            }).on('error', function(e) {
                self.emit('error', e);
            });
...
util.inherits(EccoServer, events.EventEmitter);
 ...
 */
EccoServer.prototype.start = function(cb) {
 ...
 */
EccoServer.prototype.stop = function(cb) {
    if (this.server) {
        this.server.close();
    }
    if (cb) {
